...
class PrioritizedReplayBuffer:
    ...
    def _sample_batch(self):
        ...
        # print(f"Priorities: {priorities[indices]}")
        # print(f"Mean priority: {np.mean(priorities[indices])}")
        # print(f"Median priority: {np.median(priorities[indices])}")
        # priorities = np.sort(priorities)
        # print(f"Sorted priorities: {priorities[-len(indices):]}")
        ...
        #     publ_s_list.append(episode.publ_s)
        #     priv_s_list.append(episode.priv_s) 
        #     legal_move_list.append(episode.legal_move)
        #     h0_list.append(episode.h0['h0'])
        #     c0_list.append(episode.h0['c0'])
        #     action_list.append(episode.action)
        #     reward_list.append(episode.reward)
        #     bootstrap_list.append(episode.bootstrap)
        #     terminal_list.append(episode.terminal)
        #     seq_len_list.append(episode.seq_len)

        # sample.obs['publ_s'] = torch.stack(publ_s_list).transpose(1,0).to(self.device)
        # sample.obs['priv_s'] = torch.stack(priv_s_list).transpose(1,0).to(self.device)
        # sample.obs['legal_move'] = torch.stack(legal_move_list).transpose(1,0).to(self.device)
        # sample.h0['h0'] = torch.stack(h0_list).transpose(1,0).to(self.device)
        # sample.h0['c0'] = torch.stack(c0_list).transpose(1,0).to(self.device)
        # sample.action['a'] = torch.stack(action_list).transpose(1,0).to(self.device)
        # sample.reward = torch.stack(reward_list).transpose(1,0).to(self.device)
        # sample.bootstrap = torch.stack(bootstrap_list).transpose(1,0).to(self.device)
        # sample.terminal = torch.stack(terminal_list).transpose(1,0).to(self.device)
        # sample.seq_len = torch.stack(seq_len_list).to(self.device)
        # self.indices = indices
        # return sample, weights
