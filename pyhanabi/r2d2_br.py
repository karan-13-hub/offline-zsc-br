    ...
    def td_error(
        ...
        if self.off_belief:
            ...
            if self.vdn:
                ...
                # take the last step of each batch in lstm_o according to the seq_len
                # lstm_o: [seq_len, batch, num_player, hidden_dim]
                # seq_len: [batch]
                indices = (seq_len - 1).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand(1, bsize, num_player, lstm_o.size(-1)).to(torch.int64)
                lstm_o = lstm_o.gather(0, indices).squeeze(0).sum(1)
