...
from losses import cp_loss, diversity_loss, q_ensemble_loss
import gc
import common_utils
import utils
...
if __name__ == "__main__":
    ...
    if os.path.exists(cluster_save_path) and os.path.exists(model_save_path):
        ...
        #train the mi model
        encoder, cluster_centers_tensor = train_mi(args, batch_loader=batch_loader, epochs=500, beta=0.0)
        encoder.eval()
    ...
    for epoch in range(args.num_epoch+1):
        ...
        for batch_idx, sample in enumerate(epoch_bar):
            ...
            loss = q_ensemble_loss(agent_br, diverse_agents, agent_loss_weights, batch, args)
            ...
            # Stats tracking
            stat["Ensemble loss"].feed(loss.detach().item())
        ...
        if epoch % args.num_eval_after == 0:
            ...
            print(f"Agent eval score: {score:.4f}, perfect: {perfect * 100:.2f}%")
        ...
        if epoch % args.save_model_after == 0:
            ...
            agent_model_saved = saver.save(
                None, agent_br.online_net.state_dict(), score, force_save_name=agent_force_save_name
            )
            print(f"epoch {epoch}, agent br eval score: {score:.4f}, perfect: {perfect * 100:.2f}%, model saved: {agent_model_saved}")
