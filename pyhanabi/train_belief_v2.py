...
def parse_args():
    ...
    parser.add_argument("--update_agent", type=int, default=0)
...
if __name__ == "__main__":
    ...
    for epoch in range(args.num_epoch):
        ...
        if epoch % args.update_agent == 0:
            #load a new model for the agent every 50 epochs
            # args.policy = exps/br_medium_data_seed_9_42_1234/iql_cp_bc_0.35_finetune/agent_0/model_seed_9_epoch_50.pthw
            # just change the epoch number to the epoch currently active
            policy_pth = args.policy.split("epoch_")[0] + "epoch_" + str(epoch) + ".pthw"
            if os.path.exists(policy_pth):
                agent, _ = utils.load_agent(policy_pth, agent_overwrite)
                print("\n\nloaded new model for the agent: ", policy_pth, "\n\n")
        ...
        if epoch > 0 and epoch % args.save_model_after == 0:
            force_save_name = "belief_seed_%d_epoch_%d" % (args.seed, epoch)
        saver.save(
            None,
            model.state_dict(),
            -stat["loss"].mean(),
